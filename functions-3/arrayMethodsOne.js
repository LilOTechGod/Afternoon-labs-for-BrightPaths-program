...
// CODE HERE
const myStrongest = monstersInYourPocket.filter(overCp => {
  if(overCp.CP > 200) {
    return overCp
  }
})
...
// CODE HERE
// given tax rate and price
// return a list of approriate orders
// total = price * (1 + tax)

let total = []

orders.forEach((element) => total.push(element.price * (1 + element.tax)))
// console.log(total);


function newTotal(el) {
  let total = []
  el.forEach((element) => total.push(element.price * (1 + element.tax)))
  return total;
}

const finalPrice = newTotal(orders);
...
const toysList = purchases.filter(purchases => purchases.owner === 'Bob').reduce((a, c) => a + c.price, 0)
